...
const Details = ({userData,dataLoaded}) => {
  ...
  // console.log("Details: ",userData.userData.user," ",typeof userData);
  useEffect(()=>{
    if(dataLoaded){
      setTimeout(() => {
    ...
    return (
      <>
            ...
            <div className="space-y-4">
              <Skeleton isLoaded={isLoaded} className={!isLoaded?'h-3 w-2/5 rounded-lg':''}>
                <div className="flex items-center">
                  ...
                  <span className="text-gray-700 font-body">
                    {
                      userData.user[0]?.phone_number ? userData.user[0]?.phone_number:userSelector.profile[0]?.phone_number
                    }
                  </span>
                </div>
              </Skeleton>
              <Skeleton isLoaded={isLoaded} className={!isLoaded?'h-3 w-3/5 rounded-lg':''}>
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-700 font-body">
                    {
                      userData.user[0]?.whatsapp_number ? userData.user[0]?.whatsapp_number:userSelector.profile[0]?.whatsapp_number
                    }
                    </span>
                  </div>
              </Skeleton>
              <Skeleton isLoaded={isLoaded} className={!isLoaded?'h-3 w-2/5 rounded-lg':''}>
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-700 font-body">
                    {
                      userData.user[0]?.email || userSelector.profile[0]?.email
                    }
                    </span>
                  </div>
              </Skeleton>
              <Skeleton isLoaded={isLoaded} className={!isLoaded?'h-3 w-3/5 rounded-lg':''}>
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-700 font-body">
                    {
                      userData.user[0]?.linkedin_id || userSelector.profile[0]?.linkedin_id
                    }
                    </span>
                  </div>
              </Skeleton>
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <Skeleton isLoaded={isLoaded} className={!isLoaded?'h-3 w-5/5 rounded-lg mt-2':''}>
                      ...
                      {
                        userData.user[0]?.current_address || userSelector.profile[0]?.current_address
                      }
                      </p>
                  </Skeleton>
                </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <Skeleton isLoaded={isLoaded} className={!isLoaded?'h-3 w-5/5 rounded-lg mt-2':''}>
                      <p className="text-gray-500 font-body">
                        {
                          userData.user[0]?.permanent_address || userSelector.profile[0]?.permanent_address
                        }
                      </p>
                  </Skeleton>
        ...
        <div className="mt-8 w-full lg:w-[80%] p-4 bg-gray-100 shadow-md rounded-lg">
          ...
          {
            userData.user[0]?.skills ? (
              userData.user[0]?.skills.split(",").map((skill, idx) => (
                <Skeleton key={idx} isLoaded={isLoaded} className={!isLoaded ? 'h-10 w-24 rounded-lg' : ''}>
                  <button key={idx} className="px-4 py-2 bg-primary text-white rounded-lg shadow-md hover:bg-primary transition duration-300 ease-in-out font-body">
                    {skill}
                  </button>
                </Skeleton>
              ))
